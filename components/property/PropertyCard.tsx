  ...
  return (
    <Link href={`/properties/${property.slug}`} className={cn('group block overflow-hidden rounded-lg bg-card shadow-sm border border-border transition-all hover:shadow-md', !isGrid && 'md:flex')} dir={dir}>
      <div className={cn('relative overflow-hidden', isGrid ? 'aspect-[4/3]' : 'md:w-80 md:shrink-0 h-48 md:h-auto')}>
        <img src={property.featured_image || 'https://images.unsplash.com/photo-1564013799919-ab600027ffc6?w=400'} alt={property.title} className='h-full w-full object-cover transition-transform duration-500 group-hover:scale-110' />
        <span className={cn('absolute top-3 right-3 rounded-full px-2.5 py-0.5 text-xs font-medium', badge.className)}>{badge.label}</span>
      </div>
      <div className='p-4 flex-1'>
        <h3 className='font-display text-base font-semibold leading-snug line-clamp-1'>{property.title}</h3>
        <div className='mt-1 flex items-center gap-1 text-xs text-muted'><MapPin className='h-3 w-3 shrink-0' /><span>{property.city}{property.district ? `، ${property.district}` : ''}</span></div>

...
import { ArrowLeft, MapPin, Bed, Bath, Maximize2, Calendar, Building, Layers, Ruler, FileText, CheckCircle, Send, Phone, Mail, ChevronLeft, ChevronRight, Loader2 } from 'lucide-react';
...
export default function PropertyDetailPage() {
  ...
  const allImages = property.images?.length ? property.images : [property.featured_image || 'https://images.unsplash.com/photo-1564013799919-ab600027ffc6?w=800'];
  const title = locale === 'ar' ? property.title_ar : property.title_en || property.title_ar;
  const desc = locale === 'ar' ? property.description_ar : property.description_en || property.description_ar;
  const city = locale === 'ar' ? property.city_ar : property.city_en || property.city_ar;
  const district = locale === 'ar' ? property.district_ar : property.district_en;
  const badge = getPurposeBadge(property.purpose, locale);
  const statusText = getStatusText(property.status, locale);
        ...
        <div className='grid gap-8 lg:grid-cols-3'>
          <div className='lg:col-span-2 space-y-6'>
            <div className='relative overflow-hidden rounded-xl bg-black/5'>
              <img src={allImages[imgIndex]} alt={title} className='h-80 w-full object-cover' />
              {allImages.length > 1 && (
                <div className='absolute inset-0 flex items-center justify-between px-2'>
                  <button onClick={() => setImgIndex(i => (i - 1 + allImages.length) % allImages.length)} className='rounded-full bg-white/80 p-1.5 shadow hover:bg-white'><ChevronLeft className='h-4 w-4' /></button>
                  <button onClick={() => setImgIndex(i => (i + 1) % allImages.length)} className='rounded-full bg-white/80 p-1.5 shadow hover:bg-white'><ChevronRight className='h-4 w-4' /></button>
                </div>
              )}
              {allImages.length > 1 && (
                <div className='absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1'>
                  {allImages.map((_, i) => <button key={i} onClick={() => setImgIndex(i)} className={`h-1.5 rounded-full transition-all ${i === imgIndex ? 'w-6 bg-white' : 'w-1.5 bg-white/60'}`} />)}
                </div>
              )}
            </div>
            {allImages.length > 1 && (
              <div className='flex gap-2 overflow-x-auto pb-1'>
                {allImages.map((img, i) => (
                  <button key={i} onClick={() => setImgIndex(i)} className={`shrink-0 overflow-hidden rounded-md border-2 ${i === imgIndex ? 'border-primary' : 'border-transparent'}`}>
            ...
            <div>
              ...
              <p className='mt-1 flex items-center gap-1 text-sm text-muted'><MapPin className='h-3.5 w-3.5' />{district ? `${city}, ${district}` : city}</p>
            ...
            <div className='grid grid-cols-2 gap-3 sm:grid-cols-4'>
              ...
              {property.street_width != null && <SpecItem icon={<Ruler className='h-4 w-4' />} label={t('property.street_width')} value={`${property.street_width} ${locale === 'ar' ? 'م' : 'm'}`} />}
            ...
            <div>
              ...
              <p className='mt-1 leading-relaxed text-muted'>{desc}</p>
            </div>
            ...
            {property.latitude && property.longitude && (
              <div>
                ...
                <PropertyMap latitude={property.latitude} longitude={property.longitude} title={title} />
              </div>
            )}
          </div>
          ...
          <div className='space-y-4'>
            <div className='rounded-lg border bg-card p-4'>
              ...
              {submitted ? (
                ...
                <form onSubmit={handleInquiry} className='mt-3 space-y-3'>
                  <div><input required value={inquiryForm.name} onChange={e => setInquiryForm(f => ({ ...f, name: e.target.value }))} placeholder={t('property.inquiry_name')} className='w-full rounded-md border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary' /></div>
                  <div><input required value={inquiryForm.phone} onChange={e => setInquiryForm(f => ({ ...f, phone: e.target.value }))} placeholder={t('property.inquiry_phone')} className='w-full rounded-md border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary' /></div>
                  <div><input type='email' value={inquiryForm.email} onChange={e => setInquiryForm(f => ({ ...f, email: e.target.value }))} placeholder={t('property.inquiry_email')} className='w-full rounded-md border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary' /></div>
                  <div><textarea rows={3} value={inquiryForm.message} onChange={e => setInquiryForm(f => ({ ...f, message: e.target.value }))} placeholder={t('property.inquiry_message')} className='w-full rounded-md border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary' /></div>
                  <button type='submit' disabled={submitting} className='flex w-full items-center justify-center gap-2 rounded-md bg-primary px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-primary-light disabled:opacity-60'>
            ...
            <div className='rounded-lg border bg-card p-4 space-y-3'>
              <h3 className='font-display text-base font-semibold'>{locale === 'ar' ? 'معلومات الاتصال' : 'Contact Info'}</h3>

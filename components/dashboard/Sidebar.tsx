...
import {
  ...
  ChevronRight, Activity, Mail, DollarSign, Sun, Moon, Sparkles, Search
} from 'lucide-react';
...
const navItems = [
  ...
  { separator: true, key: '' },
  { key: 'dashboard.activity', href: '/dashboard/activity', icon: Activity },
  { key: 'dashboard.email_templates', href: '/dashboard/email-templates', icon: Mail },
...
export function Sidebar() {
  ...
  return (
    <aside
      className={cn('flex flex-col border-l relative z-30 transition-all duration-300', collapsed ? 'w-[68px]' : 'w-60')}
    ...
    >
      <div className={cn('flex h-14 items-center border-b shrink-0', collapsed ? 'justify-center' : 'justify-between px-4')} style={{ borderColor: 'var(--header-border)' }}>
        {!collapsed && (
          <Link href='/dashboard' className='flex items-center gap-2.5 group'>
            <div className='flex h-8 w-8 items-center justify-center rounded-lg font-bold text-sm' style={{ background: 'var(--accent)', color: 'var(--sidebar)' }}>ر</div>
            <div>
        ...
        <button onClick={() => setCollapsed(!collapsed)} className='rounded-md p-1.5 transition-colors' style={{ color: 'var(--sidebar-fg)' }}
      ...
      {!collapsed && (
        <div className='px-3 pt-3 pb-1'>
          <div className='relative'>
            ...
            <input placeholder={t('common.search')} className='w-full rounded-lg border py-1.5 pr-8 text-xs outline-none transition-colors'
      ...
      <nav className='flex-1 overflow-y-auto p-2 space-y-0.5 scrollbar-thin'>
        {navItems.map((item, i) => {
          if ('separator' in item) return !collapsed ? <div key={`sep-${i}`} className='my-2 mx-3 h-px' style={{ background: 'var(--header-border)' }} /> : null;
          const Icon = item.icon!;
          const active = pathname === item.href || pathname.startsWith(item.href + '/');
          return (
            <Link key={item.href} href={item.href}
              className={cn('flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-all', active ? 'text-white shadow-sm' : 'hover:text-white')}
              style={{ background: active ? 'var(--sidebar-active)' : 'transparent', color: active ? 'var(--sidebar-fg-active)' : 'var(--sidebar-fg)' }}
              onMouseEnter={e => { if (!active) e.currentTarget.style.background = 'var(--sidebar-hover)'; }}
              onMouseLeave={e => { if (!active) e.currentTarget.style.background = 'transparent'; }}
            >
              <Icon className='h-4 w-4 shrink-0' style={{ color: active ? 'var(--accent)' : undefined }} />
              {!collapsed && <span className='truncate'>{t(item.key)}</span>}
              {!collapsed && active && <span className='mr-auto h-1.5 w-1.5 rounded-full shrink-0' style={{ background: 'var(--accent)' }} />}
            </Link>
          );
        })}
      </nav>
      ...
      <div className='p-2 space-y-1 shrink-0' style={{ borderTop: '1px solid var(--header-border)' }}>
        <div className='relative'>
          <button onClick={() => setShowThemeMenu(!showThemeMenu)}
            className='flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm transition-colors' style={{ color: 'var(--sidebar-fg)' }}
            onMouseEnter={e => (e.currentTarget.style.background = 'var(--sidebar-hover)')}
            onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
          >
            <ThemeIcon className='h-4 w-4 shrink-0' />
            {!collapsed && <><span className='flex-1 text-right'>{THEME_LABELS[theme].label}</span><ChevronLeft className='h-3 w-3 opacity-50' /></>}
          </button>
          {showThemeMenu && (
            <div className='absolute bottom-full right-0 left-0 mx-2 mb-1 rounded-xl border p-1.5 shadow-lg' style={{ background: 'var(--surface)', borderColor: 'var(--border)' }}>
              {(Object.entries(THEME_LABELS) as [string, typeof THEME_LABELS.light][]).map(([key, val]) => {
                const Icon = themeIcons[key];
                const isActive = theme === key;
                return (
                  <button key={key} onClick={() => { setTheme(key as any); setShowThemeMenu(false); }}
                    className='flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-xs transition-all'
                    style={{ background: isActive ? 'var(--primary-glow)' : 'transparent', color: isActive ? 'var(--primary)' : 'var(--fg-secondary)' }}
                    onMouseEnter={e => { if (!isActive) e.currentTarget.style.background = 'var(--bg-soft)'; }}
                    onMouseLeave={e => { if (!isActive) e.currentTarget.style.background = 'transparent'; }}
                  >
                    <Icon className='h-3.5 w-3.5' /><span>{val.label}</span>

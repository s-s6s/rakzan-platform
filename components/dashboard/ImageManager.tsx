...
export function ImageManager({ images, onChange, category, maxImages = 20, bucket = 'property-images' }: ImageManagerProps) {
  ...
  const uploadFiles = async (files: FileList) => {
    ...
    if (toUpload.length < files.length) { toast.warning(`يمكنك إضافة ${remaining} صور فقط`); }
    ...
    for (let i = 0; i < toUpload.length; i++) {
      ...
      if (error) { toast.error(`فشل رفع ${file.name}`); continue; }
    ...
    <div className='space-y-3'>
        ...
        <div className='grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2'>
          {images.map((url, i) => (
            <div key={i} className='group relative aspect-square overflow-hidden rounded-lg border border-border bg-muted/10'>
              <img src={url} alt='' className='h-full w-full object-cover' onError={(e) => { (e.target as HTMLImageElement).src = 'https://placehold.co/200x200/e2e8f0/94a3b8?text=خطأ'; }} />
              <div className='absolute inset-0 flex items-center justify-center gap-1 bg-black/0 opacity-0 transition-all group-hover:bg-black/40 group-hover:opacity-100'>
                <button onClick={() => removeImage(i)} className='rounded-full bg-red-500 p-1.5 text-white hover:bg-red-600 transition-colors' title='حذف'><Trash2 className='h-3.5 w-3.5' /></button>
              </div>
            </div>
          ))}
          {remaining > 0 && (
      ...
      {images.length === 0 && (
        ...
        >
          ...
          <input id={`file-${category}`} type='file' accept='image/*' multiple className='hidden' onChange={handleFileSelect} />
        </div>
      )}
